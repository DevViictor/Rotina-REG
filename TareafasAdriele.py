...
def tarefas_adriele():
    ...
    st.set_page_config(page_title="R.E.G ADRIELE", page_icon="🔒",layout="wide")
    ...
    def notificacao(mensagem):
        ...
        requests.post("https://api.pushover.net/1/messages.json", data={
            "token": api_token,
            "user": user_key,
            "message": f"Olá {"ADRIELE"}, {mensagem}"
        })
    ...
    with cola:
         st.title("📝 R.E.G - ADRIELE")

 
    # --- Função para carregar pedidos de uma aba ---
    def carregar_pedidos():
        try:
            aba = planilha.worksheet("Adriele")
    ...
    if df_consultor.empty:
        ...
        st.markdown(f"**Consultor:** Adriele")
