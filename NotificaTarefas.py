...
def notificar_page():
    ...
    st.divider()
    st.subheader("Reutilizar notifcação")
    ...
    with st.form("Forms"):
        ...
        # ----- CAMPOS -----
        titulo = st.text_input("Título da notificação :", value=titulo_default)
        descricao = st.text_input("Descrição da notifcação :", value=descricao_default)
        ...
        with col3:
            data = st.date_input("Data da notificação :")
        with col4:
        ...
        criada = st.selectbox("Criador da notificação: ", ["Victor"])
        ...
        enviar = colbtn1.form_submit_button("Enviar notificação")
